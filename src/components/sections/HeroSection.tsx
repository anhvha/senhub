  ...
  useEffect(() => {
    ...
    const drawRunner = (runner: GridRunner) => {
      ...
      gradient.addColorStop(
        0.5,
        `rgba(119, 129, 239, ${runner.opacity * 0.5})`
      )
        ...
        <div className='flex flex-col items-center text-center max-w-[800px] mx-auto'>
          ...
          <div className='inline-flex items-center gap-2 px-2 py-2 bg-[#F5F6FE] border border-transparent rounded-full mb-[18px] relative'>
            <div
              ...
              style={{
                background:
                  'linear-gradient(129deg, #7781EF 0%, #FB9A9D 100%) border-box',
                WebkitMask:
          ...
          <div className='flex flex-col items-center gap-2 w-full max-w-[500px]'>
            <div className='flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4 w-full'>
              <Button
                size='lg'
                className='w-full sm:flex-1 bg-[#4C4DDC] hover:bg-[#3f40b8] text-white px-3 sm:px-6 py-3 sm:py-4 h-auto text-sm sm:text-base font-medium rounded-[6px]'
                onClick={() => window.open(siteConfig.links.calendly, '_blank')}
              >
                {siteConfig.hero.ctaPrimary}
              </Button>

              <span className='text-[#878787] text-sm sm:text-base font-normal shrink-0'>
                Or
              </span>

              <Button
                size='lg'
                className='w-full sm:flex-1 !bg-[#303030] hover:!bg-[#404040] !text-white px-3 sm:px-6 py-3 sm:py-4 h-auto text-sm sm:text-base font-medium rounded-[8px] flex items-center justify-center gap-2 relative overflow-hidden group'
                onClick={() =>
                  window.open(siteConfig.links.installApp, '_blank')
                }
              >
                {/* Shopify Icon */}
                ...
                <span className='truncate'>{siteConfig.hero.ctaSecondary}</span>
            ...
            <p className='text-[14px] font-light text-[#959595] max-w-[400px] mt-2'>
              We&apos;ll walk you through the entire gifting workflow and show
              you real examples that fit your brand.
            </p>
          </div>
        </div>
      </Container>
